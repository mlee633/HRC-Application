...
export function ResultsDisplay({ results, isCalculating }: ResultsDisplayProps) {
  const getComplexityLevel = (score: number) => {
    if (score <= 5) return { level: "Low", color: "bg-green-500", description: "Simple medication regimen" }
    if (score <= 10) return { level: "Moderate", color: "bg-yellow-500", description: "Moderately complex regimen" }
    return { level: "High", color: "bg-red-500", description: "Complex medication regimen" }
  }
  ...
  const complexity = getComplexityLevel(results.totalScore)
  ...
  <div className="p-6">
          <div className="text-center mb-8">
            ...
            <Badge 
              className={`${complexity.color} text-white px-6 py-3 text-lg font-semibold rounded-full`}
            >
              {complexity.level} Complexity
            </Badge>
            <p className="text-slate-600 mt-3 text-lg">{complexity.description}</p>
          </div>
      ...
      <Card className="shadow-lg border-0 bg-white/80 backdrop-blur">
        ...
        <CardContent className="p-6">
          <div className="space-y-4">
            {results.medications.map((med: MCIResult['medications'][number]) => (
